...
if (empty($fname)) {
    ...
    $r = Database::search("SELECT * FROM `teacher` WHERE `email`='" . $email . "' OR `mobile` = '" . $mobile . "';");

    if ($r->num_rows > 0) {
        echo "User with the same email address or mobile already exsist";
    } else {

        Database::iud("INSERT INTO  `teacher` (`email`,`fname`,`lname`,`username`,`password`,`mobile`,`grade_id`,`subject_id`,`verify_status_id`) VALUES ('" . $email . "','" . $fname . "','" . $lname . "','".$un."','" . $password . "','" . $mobile . "','" . $grade . "','" . $sub . "','".$status."');");
        echo "success";
    }
}
