...
if (isset($_POST["e"])) {
    ...
    if (empty($email)) {
        ...
        $officer = Database::search("SELECT * FROM `officer` WHERE `email`='" . $email . "'");
        $officerN = $officer->num_rows;
        ...
        if ($officerN == 1) {
            ...
            Database::iud("UPDATE `officer` SET `verification`='" . $code . "' WHERE `email`='" . $email . "'");
